// PAGE LOADER
window.addEventListener('load',()=>setTimeout(()=>document.getElementById('pgLoader').classList.add('gone'),1400));
...
document.querySelectorAll('a,button,.btn,.skill-block,.project-card,.photo-area,.stat-item,.hero-tag,.works-row,.info-card,.nav-initials').forEach(el=>{
...
window.addEventListener('scroll',()=>{
  ...
  let cur='';
  secList.forEach(s=>{if(window.scrollY>=s.offsetTop-120)cur=s.id});
  document.querySelectorAll('.nav-links a').forEach(a=>{
    a.style.color=(a.getAttribute('href')==='#'+cur&&!a.classList.contains('nav-cta'))?'var(--accent)':'';
  });
},{passive:true});
...
document.querySelectorAll('.section-eyebrow').forEach(el=>{
  const o=new IntersectionObserver(en=>{en.forEach(e=>{if(e.isIntersecting){e.target.classList.add('ey-vis');o.unobserve(e.target);}});},{threshold:.5});
  o.observe(el);
});
  ...
  entries.forEach(e=>{
    if(e.isIntersecting){
      e.target.querySelectorAll('.skill-fill').forEach((f,i)=>{
        setTimeout(()=>{f.style.width=f.dataset.w+'%';f.classList.add('do-shimmer');},i*160);
      });
  ...
  entries.forEach(e=>{
    if(e.isIntersecting){
      const el=e.target,target=parseFloat(el.dataset.target),
            ...
            dur=1800,start=performance.now();
      function tick(now){
...
if(pCard){
  pCard.addEventListener('mousemove',e=>{
    const r=pCard.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2;
  ...
  pCard.addEventListener('mouseenter',()=>pCard.style.transition='transform .08s linear');
}
...
function uploadPhoto(e){
  ...
  r.onload=ev=>{const img=document.getElementById('heroPhoto');img.src=ev.target.result;img.style.display='block';document.getElementById('photoPlaceholder').style.display='none';};
  r.readAsDataURL(f);
}
...
function handleSubmit(e){
  ...
  btn.innerHTML='<span class="btn-shine"></span>&#10003; Message Sent Successfully';
  btn.style.background='#166534';btn.style.boxShadow='0 8px 24px rgba(22,101,52,.3)';
  setTimeout(()=>{btn.innerHTML='<span class="btn-shine"></span>Send Message';btn.style.background='';btn.style.boxShadow='';e.target.reset();},3500);
}

// MAGNETIC BUTTONS
document.querySelectorAll('.btn,.btn-submit').forEach(btn=>{
  btn.addEventListener('mousemove',e=>{
    const r=btn.getBoundingClientRect(),dx=(e.clientX-r.left-r.width/2)*.22,dy=(e.clientY-r.top-r.height/2)*.22;
